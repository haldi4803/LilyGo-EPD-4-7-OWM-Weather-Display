...
long ShortSleepDuration = 5; // Sleep time in minutes, aligned to the nearest minute boundary, so if 30 will always update at 00 or 30 past the hour
int  WakeupHour      = 7;  // Wakeup after 06:00 to save battery power
int  SleepHour       = 17; // Sleep  after 24:00 to save battery power
long StartTime       = 0;
long SleepTimer      = 0;
long ShortSleepTimer = 0;
long Delta           = 30; // ESP32 rtc speed compensation, prevents display at xx:59:yy and then xx:00:yy (one minute later) to save power
int notime           = 0; //needed for getting Time.
...
#include "opensans48b.h" // Numbers only!!!
#include "moon.h"
#include "sunrise.h"
#include "sunset.h"
#include "uvi.h"
...
void BeginShortSleep() {
  epd_poweroff_all();
  UpdateLocalTime();
  ShortSleepTimer = (ShortSleepDuration * 60 - ((CurrentMin % ShortSleepDuration) * 60 + CurrentSec)) + Delta; //Some ESP32 have a RTC that is too fast to maintain accurate time, so add an offset
  esp_sleep_enable_timer_wakeup(ShortSleepTimer * 1000000LL); // in Secs, 1000000LL converts to Secs as unit = 1uSec
  Serial.println("Awake for : " + String((millis() - StartTime) / 1000.0, 3) + "-secs");
  Serial.println("Entering " + String(ShortSleepTimer) + " (secs) of sleep time");
  Serial.println("Starting SHORT deep-sleep period...");
  esp_deep_sleep_start();  // Sleep for e.g. 30 minutes
}
boolean SetupTime() {
...
uint8_t StartWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("WiFi connection *** FAILED *** Starting AP Mode"); //KEIN WIFI Start AP Mode!
    NoWiFiTurnOff();
  }
...
void setup() {
  ...
  if (notime == 1) {   // NO TIME, connect to WiFi or start AP Mode
    ...
    if (WakeupHour > SleepHour)
      ...
      WakeUp = (CurrentHour >= WakeupHour && CurrentHour <= SleepHour);
      InitialiseSystemSCREEN(); //With Screen
    if (CurrentMin != 00 && rtc_get_reset_reason(0) != POWERON_RESET && rtc_get_reset_reason(0) != SW_CPU_RESET ){ //Do Clock Only refresh if not XX:00 clock and NOT reset by Powerbutton AND NOT reset by SW_Reset after obtaining time.
      Serial.println("Nicht Volle Stunde! Nur Partial Refresh der Uhr Ohne WiFi");
      epd_poweron();      // Switch on EPD display
      DisplayClock(); // Update Clock Only
      edp_update();  
      epd_poweroff_all(); // Switch off all power to EPD
      BeginShortSleep(); // Short Sleep timer for Clock Only
    }
    if (WakeUp) {
      if (StartWiFi() == WL_CONNECTED && SetupTime() == true) {
      byte Attempts = 1;
      ...
      if (RxWeather && RxForecast) { // Only if received both Weather or Forecast proceed
        ...
        epd_poweroff_all(); // Switch off all power to EPD
        BeginShortSleep(); // NEEDS to be here to prevent long Sleep!
      }
    }
  }
      else
      Serial.println("Zwischen Awake Time!");
      //Going back to Deep Sleep long timer.
      BeginSleep();
...
void NoWiFiTurnOff(){
  ...
  InitialiseSystemSCREEN();
  epd_poweron();      // Switch on EPD display
  epd_clear();        // Clear the screen
  setFont(OpenSans24B);
  drawString(490, 200, "No WiFi Connection", CENTER);
  drawString(490, 240, "Entering Deep Sleep", CENTER);
  drawString(490, 310, "Press WakeUP Button", CENTER);
  DrawSegment(500, 20, 0, 0, 20, -20, 20, -20, 40, 0);
  DrawSegment(500, 28, 0, 0, 20, -20, 20, -20, 40, 0);
  edp_update();       // Update the display to show the information
  epd_poweroff_all(); // Switch off all power to EPD
  Serial.println("DeepSleep Infinite"); 
  esp_sleep_enable_ext0_wakeup(GPIO_NUM_39,0); //Wakeup by button most right.
...
void DisplayClock(){
  ...
  area.width = 255;
  area.height = 80;
  epd_clear_area_cycles(area, 6, 150);
  DisplayClockSection(480, 75);                         // Clock in BIG
}
...
void DisplayClockSection(int x, int y) {
  setFont(OpenSans48B);
  drawString(x, y, Time_str, LEFT);
}
...
void DisplayTempHumiPressSection(int x, int y) {
  ...
  setFont(OpenSans12B);
  DrawPressureAndTrend(x + 215, y - 10, WxConditions[0].Pressure, WxConditions[0].Trend); //changed  y + 15 to y - 0 to make space for CLOCK and x +195 to x + 215 to center
  int Yoffset = 42;
  ...
  drawString(x - 30, y + Yoffset, String(WxConditions[0].High, 0) + "° | " + String(WxConditions[0].Low, 0), LEFT); // Show forecast high and Low
}
...
void DrawPressureAndTrend(int x, int y, float pressure, String slope) {
  drawString(x + 25, y - 10, String(pressure, (Units == "M" ? 0 : 1)) + (Units == "M" ? "mbar" : "in"), LEFT);
  if      (slope == "+") {
